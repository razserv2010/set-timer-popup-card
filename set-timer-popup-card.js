...
class SetTimerCard extends LitElement {
  ...
  static styles = css`
    .set-timer-card { overflow: hidden; height: 100%; }
    .container ha-card { border: none !important; padding: 12px; }
    .timer-input-card { display: flex; align-items: center; flex-direction: column; gap: 15px; border: none !important; }
    .timer-input-wrapper { display: flex; justify-content: center; flex-direction: column; align-items: center; gap: 8px; }
    .dimmed { opacity: 0.9; }
    .timer-setting-text { font-size: 17px; }
    .column-titles { display: flex; }
    .column-title { width: 90px; text-align: center; font-family: Arial, sans-serif; }
    .timer-columns-wrapper { width: fit-content; display: flex; align-items: center; }
    ...
    .timer-digit { text-align: center; min-width: 85px; min-height: 55px; }
    .digit-seperator { width: 4px; height: 130px; background-color: var(--primary-text-color); }
    .timer-action-selector { display: flex; align-items: center; z-index: 5; gap: 8px; }
    ...
    .set-timer-button { padding: 10px 16px; background-color: var(--primary-color); color: white; border: none; border-radius: 4px; z-index: 5; cursor: pointer; font-size: 1rem; }
  `;
          ...
          <div class="card-content timer-input-card">
            <div class="timer-input-wrapper ${this.entityState == "set" ? "dimmed" : ""}">
              ...
              <div class="timer-columns-wrapper">
                <div class="timer-digit-column-wrapper" id="hours-column"
                     @touchstart="${this._handleTouchStart}" @touchmove="${this._handleTouchMove}" @wheel="${this._handleScroll}">
                  <div class="timer-digit-column">
                    <div class="timer-digit"></div>
                    ${Array.from({length:24}, (_,i)=>html`<div class="timer-digit">${String(i).padStart(2,"0")}</div>`)}
                  </div>
                </div>
                <div class="digit-seperator"></div>
                <div class="timer-digit-column-wrapper" id="minutes-column"
                     @touchstart="${this._handleTouchStart}" @touchmove="${this._handleTouchMove}" @wheel="${this._handleScroll}">
                  <div class="timer-digit-column">
                    <div class="timer-digit"></div>
                    ${Array.from({length:60}, (_,i)=>html`<div class="timer-digit">${String(i).padStart(2,"0")}</div>`)}
                  </div>
                </div>
                <div class="digit-seperator"></div>
                <div class="timer-digit-column-wrapper" id="seconds-column"
                     @touchstart="${this._handleTouchStart}" @touchmove="${this._handleTouchMove}" @wheel="${this._handleScroll}">
                  <div class="timer-digit-column">
                    <div class="timer-digit"></div>
                    ${Array.from({length:60}, (_,i)=>html`<div class="timer-digit">${String(i).padStart(2,"0")}</div>`)}
  ...
  _handleTouchMove(event) {
    ...
    if (this.entityState == "idle") {
      ...
      let scrollDirectionUpward = deltaY > 0;
  ...
  _updateRemaningTime(finishingAt) {
    ...
    // יישור לאינדקסי ה־UI (יש רווח ריק בשורה הראשונה)
  ...
  _setTimerAction(clickEvent) {
    if (this.entityState == "idle") {
      ...
      // שולחים את הערך החוקי לשירות: turn_on/turn_off/toggle
      this.timerAction = button.id;
    }
  }
  ...
  _submitAction(clickEvent) {
    if (this.entityState == "idle") {
      ...
      // המרת אינדקסים לערכים בפועל (בגלל השורה הריקה העליונה)
      ...
      this._hass.callService("switch_timer", "set_timer", {
        ...
        action: actionToSend,
        duration: `${pad(hVal)}:${pad(mVal)}:${pad(sVal)}`, // פורמט HH:MM:SS
      });
  ...
  setConfig(config) {
    if (!config.entity) {
      ...
      throw new Error("The supplied entity is not a valid 'switch_timer' entity");
    }
